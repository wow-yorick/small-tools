...
function get_message_list($user_id, $user_name, $num, $start, $order_id = 0)
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //{
            $reply = array();
            $sql   = "SELECT user_name, user_email,contact,company,phone, msg_time, msg_content".
            ...
            if ($reply)
            {
                ...
                $msg[$rows['msg_id']]['re_msg_content'] = nl2br(htmlspecialchars($reply['msg_content']));
				  $msg[$rows['msg_id']]['re_contact'] = $reply['contact'];
				    $msg[$rows['msg_id']]['re_company'] = $reply['company'];
					  $msg[$rows['msg_id']]['re_phone'] = $reply['phone'];
            }
        //}
...
function add_message($message)
{
    ...
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('feedback') .
            " (msg_id, parent_id, user_id, user_name, user_email, msg_title, msg_type, msg_status, phone,company, contact, msg_content, msg_time, message_img, order_id, msg_area)".
            " VALUES (NULL, 0, '$message[user_id]', '$message[user_name]', '$message[user_email]', ".
            " '$message[msg_title]', '$message[msg_type]', '$status', '$message[msg_content]', '".gmtime()."', '$img_name', '$message[order_id]', '$message[msg_area]', '$message[phone]', '$message[company]', '$message[contact]')";
