...
//lyf->
            ...
            "WHERE a.cat_id = " . intval($cat_id) ." AND v.attr_value IS NOT NULL AND a.attr_values IS NOT NULL AND a.sort_order = '200' OR a.cat_id = 0 ".

            "ORDER BY a.sort_order, a.attr_type, a.attr_id, v.attr_price, v.goods_attr_id";



    $row1 = $GLOBALS['db']->GetAll($sql);

    $sql1 = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price ".
            ...
            "WHERE a.cat_id = " . intval($cat_id) ." AND a.sort_order <> 200 OR a.cat_id = 0 ".

            "ORDER BY a.sort_order, a.attr_type, a.attr_id, v.attr_price, v.goods_attr_id";



    $row2 = $GLOBALS['db']->GetAll($sql1);
       ...
       if($filter['cat_id']>0){ 
...

        {
            ...
            case 'all_type';
                ...
                $where .= " AND (is_best=1 OR is_hot=1 OR is_taobao=1 OR is_new=1 OR (is_promote = 1 AND promote_price > 0 AND promote_start_date <= '" . $today . "' AND promote_end_date >= '" . $today . "'))";
        ...
        $sql = "SELECT g.goods_id, g.goods_name, g.shop_title, g.nick, g.goods_weight, g.goods_type, g.goods_sn,  g.shop_price, g.commission_rate*100 AS commission_rate, " . "g.goods_thumb" . ", g.click_url, g.original_img, g.commission, g.commission_num, g.is_on_sale, g.is_best, g.is_taobao, g.is_new, g.is_hot, g.sort_order, g.goods_number, g.integral, " .
...
//牛模式（lyf）添加start    
...

    {
        ...
        $goods_id = $row[$key]['goods_id'];

		$img_url = $GLOBALS['db']->getOne('SELECT img_url FROM ' .$GLOBALS['ecs']->table('goods_gallery'). " WHERE goods_id='$goods_id'");

		if (is_int(strpos($img_url,'http://'))) {
...

{
    ...
    $sql = "SELECT g.goods_attr_id, g.attr_value, g.attr_id, a.attr_name
            ...
            ORDER BY g.goods_attr_id ASC";//yxn修正商品属性排序
...
//yxn增加：属性价格pro_price//
        ...
        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, pro_price, product_number
