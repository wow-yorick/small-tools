	...
	function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		...
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			...
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				...
				$colwidths = $this->colwidth($col,$sheet)*1;
				for($i=0;$i<$rowspan;$i++) {
					for($j=0;$j<$colspan;$j++) {
						if ($i>0 || $j>0) {
							$this->sheets[$sheet]['cellsInfo'][$row+$i][$col+$j]['dontprint']=1;
							$colwidths = $colwidths + $this->colwidth($col+$j,$sheet)*1;
						}
					}
				}
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					...
					$cellid = strtoupper($this->colindexes[$col]) . $row;
					$out .= "\n\t\t<td id='" . $cellid . "' style=\"$style\"" . ($colspan > 1?" colspan=$colspan":"") . ($rowspan > 1?" rowspan=$rowspan":"") . ">";
