...
class interface_single_casdict // !--- MODIFY --- //
{
			...
			'searchmenu' => array( //搜索操作按钮定义
				...
				'items' => array(
					...
					'type' => array(
						...
						'class' => 'wiliz-slt',
						'colspan' => '1',
						'datasource' => $this->dbAll->all_getGlobalType("级联配置类型"),
						'valField' => 'name',
						'txtField' => 'name',
						'attr' => array(),
					),
	...
	public function getGridDefine()
	{
		$gridinfo = array(
			...
			'listfield' => array( //显示字段
				...
				array(
					'fieldname' => 'fromtype',			//需要和sql语句中的字段名完全一致
					'dispname' => '来源类型',			//中文字段信息
				),
				array(
					'fieldname' => 'fromvalue',
					'dispname' => '来源值',
