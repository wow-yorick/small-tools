...
class Systemsetting_BackupController extends Zend_Controller_Action // !--- MODIFY --- //
{
	...
	public function backupAction(){
		require_once APPLICATION_PATH . '/models/dbbackup.class.php'; // !--- COPY --- //
		...
		$dataInfo["filename"] = $dbbackup->filename;
		$dataInfo["backupdate"] = date('Y-m-d G:i:s');
		$dataInfo["desc"] = $dbbackup->tips;
	...
	public function restoreAction(){
		$data = $this->db->get($_POST["idString"]);
		require_once APPLICATION_PATH . '/models/dbbackup.class.php'; // !--- COPY --- //
		$this->logger->debug("DEBUG :".Zend_Json::encode($data));
		$dbbackup = new dbbackup();
		$dbbackup->conn();
		$dbbackup->restore($data["obj"]["fiilename"]);
		$this->view->menuId = 'li' . $this->view->moduleName . $this->view->controllerName;
		echo $this->view->render('/all/views/refresh_ajax.phtml');
	...
	public function showaddAction()
	{
		...
		// !---------- BEGIN: CUSTOM ---------- //
		$this->view->allGTG = Ousu::all_html_option($this->dbAll->all_getGlobalConfig(),"groupName","groupName"); //获取字典表中的所有组
				// !---------- END  : CUSTOM ---------- //
