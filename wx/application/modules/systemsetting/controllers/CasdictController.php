...
class Systemsetting_CasdictController extends Zend_Controller_Action // !--- MODIFY --- //
{
	...
	{	
		// !---------- BEGIN: CUSTOM ---------- //
		$params = array (
			"0" => array("name"=>"sys_dictdata"),
			"1" => array("name"=>"sys_casdictdata")
		);
		$this->view->allFROM = Ousu::all_html_option($params,"name","name"); //获取字典表中的所有组
		if (isset($this->view->dataDetail)){
			$select = array();
			$select["fromtype"] = "<option selected value='".$this->view->dataDetail["fromtype"]."'>".$this->view->dataDetail["fromtype"]."</option>";
			$select["fromvalue"] = "<option selected value='".$this->view->dataDetail["fromvalue"]."'>".$this->view->dataDetail["fromvalue"]."</option>";
		}
		$this->view->allSel = $select;
		$this->view->allTYPE = Ousu::all_html_option($this->dbAll->all_getGlobalType("级联配置类型"),"name","name"); //获取字典表中的所有组
		// !---------- END  : CUSTOM ---------- //
	}
	...
	public function getselectAction(){
		$params = array();
		$params["casdata"] = $_GET["casdata"];
		$params["type"] = $_GET["type"];
		if ($params["type"] == "fromtype" ){
			if ($params["casdata"] == "sys_dictdata"){
				$sql = "SELECT name from sys_dictdata where type='字典类型'";
				$result = $this->db->find($sql);
				echo ousu::all_html_option($result["obj"],"name","name");
			}
			else {
				$sql = "SELECT type from sys_casdictdata group by type";
				$result = $this->db->find($sql);
				echo ousu::all_html_option($result["obj"],"type","type");			
			};
			ousu::setSessionArray("casdictfrom", $params);
		}
		else {
			$arr = ousu::getSessionArray("casdictfrom");
			if ($arr["casdata"] == "sys_dictdata"){
				$sql = "SELECT name,value from sys_dictdata where type='".$params["casdata"]."'";
				$result = $this->db->find($sql);
				echo ousu::all_html_option($result["obj"],"name","name");
			}
			else {
				$sql = "SELECT name,value from sys_casdictdata where type ='".$params["casdata"]."'";
				$result = $this->db->find($sql);
				echo ousu::all_html_option($result["obj"],"name","name");			
			};
		}
	...
	public function addAction()
	{
		...
		$dataInfo["createuser"] = $this->loginUser;
	...
	public function modifyAction()
	{
		...
		$dataInfo["modifyuser"] = $this->loginUser;
