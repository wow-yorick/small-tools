...
class interface_single_itemdefine // !--- MODIFY --- //
{
	public function getMenuDefine()
	{
		$this->dbAll = new db_all(); //用于获取select标签的datasource
		//获取db实例
		$arrTmp = explode('_', __CLASS__);
		$className = 'db_' . $arrTmp[2];
		...
		$menuinfo = array(
			...
			'searchmenu' => array( //搜索操作按钮定义
				...
				'items' => array(
					'moduleId' => array(
						...
						'class' => 'wiliz-sltl',
						'colspan' => '1',
						'datasource' => $this->db->custom_getAllModule(),
						'valField' => 'moduleId',
						'txtField' => 'displayName',
					),
					'controllerId' => array(
						'name' => 'controllerId',
						...
						'class' => 'wiliz-sltl',
						'colspan' => '1',
						'datasource' => $this->db->custom_getAllController(),
						'valField' => 'controllerId',
						'txtField' => 'displayName',
						'attr' => array(
							'disabled' => 'disabled'
						),
			...
			'othermenu' => array( //其它操作按钮定义
				'btnItemSort' => array(
					'itemname' => 'btnItemSort',
					'imgname' => 'sort',
					'dispname' => '模块排序',
					'iscustomclick' => '0',
					'onclick' => 'custom_showItemSortTab(this);',
					'actionname' => 'showitemsorttab',
					'actionname_show' => '',
					'isalert' => '0',
					'alertval' => '',
				),
			...
			'menutoggle' => array( //按钮间相互enable、diable关系，自己追加othermenu中的按钮动作
				'init' => array('btnAdd', 'btnSearch', 'btnItemSort'),	//页面初始化时
				'gridclick' => array('btnModify', 'btnDetail'),			//点击列表行时
				...
				'btnDetail' => array('btnCancel'),						//点击详细按钮时
				'btnDelete' => array('btnAdd'),							//点击删除按钮时
				'btnItemSort' => array('btnCancel'),					//点击模块排序按钮时 // !--- CUSTOM --- //
			),
		);
		return $menuinfo;
	}
	...
	public function getGridDefine()
	{
		$gridinfo = array(
			...
			'listfield' => array( //显示字段
				/*array( //序号列，直接复制，位置与$menuinfo中的No_colNum值对应
					'fieldname' => null,
					'dispname' => 'NO.',
				),*/
				array(
					"fieldname" => "moduleItemName",
					"dispname" => "ModuleItemName",
				),
				array(
					'fieldname' => 'itemname',		//需要和sql语句中的字段名完全一致
					'dispname' => 'ItemName',		//中文字段信息
				),
				array(
