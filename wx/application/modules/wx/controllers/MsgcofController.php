...
class Wx_MsgcofController extends Zend_Controller_Action // !--- MODIFY --- //
{
	...
	{	
		// !---------- BEGIN: CUSTOM ---------- //
		$this->view->allGTG = Ousu::all_html_option($this->dbAll->all_getGlobalType('信息分类'),"name","name"); //获取字典表中的所有组
		// !---------- END  : CUSTOM ---------- //
	}
	...
	private function getAddModifySameData()
	{
		...
		// !---------- BEGIN: MODIFY ---------- //
		$dataInfo['msgid'] = md5(time());
		$dataInfo['modifytime'] = date('Y-m-d H:i:s');
		// !---------- END  : MODIFY ---------- //
