...
namespace Think;
	 ...
	 *      $auth=new group();  $auth->check('规则名称','用户id')
	 * 2，可以同时对多条规则进行认证，并设置多条规则的关系（or或者and）
	 *      $auth=new group();  $auth->check('规则1,规则2','用户id','and')
...
class Auth
{
	...
	protected function getAuthList($uid, $type)
	{
		...
		//读取用户组所有权限规则
		$rules = M()->table($this->_config['AUTH_RULE'])->where($map)->field('controller_name,action_name')->select();
		//循环规则，判断结果。
		$authList = array();
		foreach ($rules as $rule) {
			if (!empty($rule['condition'])) { //根据condition进行验证
				...
				if ($condition) {
					$authList[] = strtolower($rule['controller_name'].'/'.$rule['action_name']);
				}
			} else {
				//只要存在就记录
				$authList[] = strtolower($rule['controller_name'].'/'.$rule['action_name']);
